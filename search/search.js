function search() {
 ...
 /* 
            var articleData = $( "entry", jsonResponse ).map(function(item) {
                return {
...
*/
            ...
            var articleData = jsonData.map(function(item) {
                return {
                    title: item.title,
            ...
            articleData.forEach(function(data) {
                ...
                if (keywords[0]) {
                    if (isMatch) {
                        post_str += "<li class='article-result-item'><a href='.." + data_url + "index.html' target='_blank' class='search-result-title'>" + data_title + "</a>";
...
/*
            ...
            $comment_count.html("搜索到 " + comment_count + " 条评论");
*/
        }
    });
}